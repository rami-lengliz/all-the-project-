...
export class LedgerService {
  ...
  async postCapture(
    ...
    if (existing) {
      this.logger.log(
        `postCapture: already posted for paymentIntentId=${paymentIntentId}, skipping.`,
      );
  ...
  async postRefund(
    ...
    if (originals.length === 0) {
      this.logger.warn(
        `postRefund: no capture entries found for paymentIntentId=${paymentIntentId}`,
      );
      return { entries: [], idempotent: true };
    }

    // Idempotency check: already reversed?
    const alreadyReversed = originals.every(
      (e) => e.status === LedgerStatus.REVERSED,
    );
    if (alreadyReversed) {
      this.logger.log(
        `postRefund: already reversed for paymentIntentId=${paymentIntentId}, skipping.`,
      );
      const refundEntries = await db.ledgerEntry.findMany({
        where: { paymentIntentId, bookingId, type: LedgerEntryType.REFUND },
      });
      return {
        entries: refundEntries.map((e) => ({
          id: e.id,
          type: e.type,
          direction: e.direction,
          amount: e.amount.toString(),
    ...
    for (const orig of originals) {
      ...
      const reversal = await db.ledgerEntry.create({
        data: {
          ...
          direction:
            orig.direction === LedgerDirection.CREDIT
